...
//			array( "name" => "record_controls_block", "align" => "left" ) 
	...
	protected function getHeaderControlsBlocks()
	{
		...
		if( $this->inlineEditAvailable() && $this->xt->getVar("editselected_link") )
		{
			...
			if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT) 
				$buttons.= '<a class="rnr-button bs-invisible-button" href="#" '.$editselectedlink_attrs.' '.$editselectedlink_span.'>'.mlang_message("AA_P_EDIT").'</a> ';
			else
				$buttons.= '<a class="' . $bs_button_class . ' bs-invisible-button" href="#" '.$editselectedlink_attrs.' '.$editselectedlink_span.'>'.mlang_message("AA_P_EDIT").'</a> ';
		};	
		...
		if( $this->deleteAvailable() && $this->xt->getVar("deleteselected_link") )
		{
			...
			// "bs-invisible-button" class need for init hidden in bootstrap
			if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT) 
				$buttons.= '<a class="rnr-button bs-invisible-button" href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'.mlang_message("AA_P_DELETE").'</a> ';		  
			else
				$buttons.= '<a class="' . $bs_button_class . ' bs-invisible-button" href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'.mlang_message("AA_P_DELETE").'</a> ';		  
		}		
