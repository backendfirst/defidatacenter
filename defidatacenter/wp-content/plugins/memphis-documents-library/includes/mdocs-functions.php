...
function enviar_notificacion_cliente ($filename,$name,$desc,$cat,$modified) {
	// $filename
	// $name
	// $desc
	// $cat
	...
	// ddc *** Recuperar informacion para el correo
	...
	$users_cliente = [];
	$users_cliente =   get_users( [ 'blog_id=$id_empresa_cliente&role=cliente']);
					// lista de usuarios a los que se enviara la notificacion: 
					// todos los clientes de la empresa donde se dio de alta el documento

	$FechaCadena = date_format($modified, 'd-m-Y H:i');
	$total_users = count($users_cliente);
    if ($total_users>0) {

		foreach ($users_cliente as $usuario_cliente) {
            $to =$usuario_cliente->user_email;
            $headers = "Content-Type: text/html; charset=iso-8859-1\n";
            $headers .= "From:".$email_coordinador."\r\n";

            $tema="Un nuevo archivo de DefiDataCenter.";

            $mensaje="
			<table border='0' cellspacing='2' cellpadding='2'>
			<tr>
			<td width='20%>'</td>
			<td width='60%' align='left'>Estimado usuario:La plataforma de documentos Defidatacenter, te ha enviado un archivo:</td>
			<td width='20%>'</td>
			</tr>
			</table>
			<br>
			<table>
			<tr>
			<td width='20%' align='center' bgcolor='#FFFFCC'><strong>Archivo:</strong></td>
			<td width='80%' align='left'>\"$name\"<br>\"$filename\"</td>
			</tr>
			<tr>
			<td align='center' bgcolor='#FFFFCC'><strong>Descripcion:</strong></td>
			<td align='left'>$desc</td>
			</tr>
			<tr>
			<td width='20%' align='center' bgcolor='#FFFFCC'><strong>Carpeta:</strong></td>
			<td width='80%' align='left'>$cat</td>
			</tr>
			<tr>
			<td align='center' bgcolor='#FFFFCC'><strong>Fecha:</strong></td>
			<td align='left'>$FechaCadena</td>
			</tr>
			</table>
			<br>
			Atte.<br>
...
function get_usuarios() { 
    ...
    foreach ($perfiles as $perfil) :
        ...
        if ($resultados) $usuarios = array_merge($usuarios, $resultados);
    endforeach;
