...
function mdocs_file_upload() {
	...
	$mdocs_name = $_POST['mdocs-name'];   			// *** Nombre del Documento
	...
	$mdocs_cat = $_POST['mdocs-cat'];    			// *** Sera esta la Carpeta donde se guarda??
	$mdocs_desc = wpautop($_POST['mdocs-desc']);  	// *** Descripcion del documento
	$mdocs_version = $_POST['mdocs-version'];  		// *** Version del documento 
	...
	$mdocs_user = $current_user->user_login;   		// *** nombre de usuario que lo subio 
				...
				if($mdocs_type == 'mdocs-add') {
					if($valid_mime_type) {
						...
						if(!isset($upload['error'])) {
							...
							enviar_notificacion_cliente ($upload['filename'],$upload['name'],$upload['desc'],$mdocs_cat,$upload['modified']);  /// ddc *** enviar notificacion al cliente
							mdocs_save_list($mdocs);   		///  *** aqui se guarda
					...
					if($_FILES['mdocs']['name'] != '') {
						...
						$date = mdocs_format_date($_POST['mdocs-last-modified']); //  ***fecha 
						...
						$mdocs = mdocs_array_sort($mdocs);
						mdocs_save_list($mdocs);    ///  *** aqui se guarda
						enviar_notificacion_cliente ($mdocs['filename'],$mdocs['name'],$mdocs['desc'],$mdocs['cat'],$mdocs['modified']);  /// ddc *** enviar notificacion al cliente
					}
				}
