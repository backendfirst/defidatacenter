...
class QMNQuizManager {
	 ...
	 * @return void
	 */
	public $ddc_sitio_original;

	public function __construct($sitio_origen) {
		$this->ddc_sitio_original = $sitio_origen;
		$this->add_hooks();
	...
	public function submit_results( $qmn_quiz_options, $qmn_array_for_variables ) {
		...
		$rutaorigen = $_COOKIE["RUTA_ORIGINAL"];
		if (is_null($rutaorigen)){
			$rutaorigen = "http://defidatacenter.info/ddc/rutadesuempresa";
		};
	...
	public function send_admin_email($qmn_quiz_options, $qmn_array_for_variables)
	{
		...
		if ( $qmn_quiz_options->send_admin_email == "0" ) {
			...
			if ( get_option('mlw_advert_shows') == 'true' ) {$mlw_message .= "<br>(c) 2018 QSM by FC, Defidatacenter.";}
			$headers = array(
...
$ruta = esc_url($_GET['ruta_original']);
setcookie("RUTA_ORIGINAL" ,$ruta,0,"/");
//echo esc_url($_COOKIE["RUTA_ORIGINAL"])."/".$ruta;

$qmnQuizManager = new QMNQuizManager($ruta);
