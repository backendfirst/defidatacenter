...
function fca_qc_save_post( $post_id ) {
	...
	if ( array_key_exists ( 'fca_qc_quiz_preview_url', $_POST ) ) {
		...
		//SAVING QUESTIONS
		update_post_meta ( $post_id, 'quiz_cat_questions', json_decode( stripslashes_deep( fca_qc_kses_html( $_POST['fca_qc_questions_json'] ) ), true ) );
		...
		if ( $quiz_type == 'pt' && function_exists ('fca_qc_save_personality_results') ) {
			fca_qc_save_personality_results( $post_id, $_POST );
		} else {
